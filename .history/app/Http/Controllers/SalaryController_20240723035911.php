...
class SalaryController extends Controller
{
    ...
    public function getViewIndexSlipGajiByMonthYear(Request $request) : View
    {
        ...
        if(true) {
            $dataManpower = Manpower::where('id' Auth()->user())->with('salary')->with(['dcurecap' => function ($query) use ($pattern) {
                // $pattern = '2024-07-\d{2}$';
                // $pattern = $pattern;
                $query->whereRaw('tanggal REGEXP ?', [$pattern]);
            }])->get();
    ...
    public function approvePendapatanGaji(Request $request)
    {
        $dataSalaryPerson = Salary::where('manpower_id', $request->id)->first();
    ...
    public function cancelApprovePendapatanGaji(Request $request)
    {
        $dataSalaryPerson = Salary::where('manpower_id', $request->id)->first();
        $confirmSalaryPerson = ConfirmSalary::where('salary_id', $dataSalaryPerson->id)->first();
