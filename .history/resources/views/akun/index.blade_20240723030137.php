...
@section('konten')
    ...
    <div class="card mx-auto p-5 mt-5" style="width: 60%">
        ...
        <hr> --}}
        <div>
            <form action="{{ (Auth::user()->level === 'pekerja') ? route('updateProfilePersonPending', Auth::user()->manpower_id) : route('manpower.update', $data->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                {{-- input nama pekerja --}}
                <div class="mb-3">
                    <label for="inputPasswordLama" class="form-label"><h5 class="mb-0">Ubah Password Akun</h5></label>
                    <input class="mt-2" type="password" class="form-control" id="inputPasswordLama" name="inputPasswordLama" placeholder="Password Lama">
                    <input class="mt-2" type="password" class="form-control" id="inputPasswordBaru" name="inputPasswordBaru" placeholder="Password Baru">
                    <input class="mt-2" type="password" class="form-control" id="inputPasswordBaru" name="inputPasswordBaru" placeholder="Konfirmasi Password Baru">
                </div>
