                ...
                @if(auth()->user()->level == 'hrd') 
                    <td>
                        <div id="btnAksi{{ $dataSalary->manpower->id }}"> 
                            @if($confirmSalaryPerson->status_gaji == true)
                                <button class="btn btn-sm btn-danger" onclick="aksiCancelApprove({{ $dataSalary->manpower->id }})" style="width: 100%; display: flex; align-items: center; justify-content: space-evenly"><span class="material-symbols-outlined">delete_forever</span>Cancel Approve</button>
                            @else
