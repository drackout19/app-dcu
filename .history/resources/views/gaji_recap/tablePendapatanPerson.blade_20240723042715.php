                ...
                @if(auth()->user()->level == 'hrd') 
                    <td>
                        <div id="btnAksi{{ $dataSalary->manpower->id }}"> 
                            ...
                            @if($confirmSalaryPerson->konfirmasi_keuangan == true && Auth::user()->level == 'keuangan')
                                <button class="btn btn-sm btn-danger" onclick="aksiCancelApproveBy({{ $dataSalary->manpower->id }})" style="width: 100%; display: flex; align-items: center; justify-content: space-evenly"><span class="material-symbols-outlined">delete_forever</span>Cancel Approve</button>
                            @else
                            <span class="btn btn-sm btn-primary" style="width: 100%; display: flex; align-items: center; justify-content: space-evenly" onclick="aksiEdit({{ $dataSalary->manpower->id }})"><span class="material-symbols-outlined fs-6">edit</span>Edit</span>
                            @endif
