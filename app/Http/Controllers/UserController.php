...
class UserController extends Controller
{
    ...
    public function updateAkunPekerja(Request $request, $id) : redirectResponse
    {
        ...
        if (!Hash::check($request->inputPasswordLama, $user->password)) {
        ...
        $user->password = bcrypt($request->inputPasswordBaru);
        $user->save();

        return redirect()->route('akunPekerja', ["id" =>  $id])->with(['success' => 'Data berhasil Diubah!']);
